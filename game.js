...
function spawnKiwi() {
  ...
  function hitKiwi(touchX, touchY) {
    ...
    // ถ้ามือถือใช้จุดที่นิ้วแตะ ถ้า desktop ใช้กลางนก
    const cx = (touchX !== null ? touchX : r.left + r.width/2)  - ar.left;
    const cy = (touchY !== null ? touchY : r.top  + r.height/2) - ar.top;
  ...
  // touch — ใช้ตำแหน่งนิ้วจริง
  kiwi.addEventListener('touchstart', e => {
    e.preventDefault();
    e.stopPropagation();
...
/* ══════════════════════════════════════
   BOTTOM BAR (มือถือเล็ก)
══════════════════════════════════════ */
let _currentTab = 'item';

function renderBottomBar() {
  const bc = document.getElementById('bottomContent');
  if (!bc) return;
  if (_currentTab === 'item') {
    bc.innerHTML = document.getElementById('itemList').innerHTML;
  } else {
    bc.innerHTML = document.getElementById('questList').innerHTML;
  }
}
...
function switchTab(tab) {
  ...
  document.getElementById('tabQuest').classList.toggle('active', tab === 'quest');
  renderBottomBar();
}

/* patch renderItems/renderQuests ให้ sync bottom bar ด้วย */
const _ri = renderItems;
const _rq = renderQuests;
renderItems  = function() { _ri(); renderBottomBar(); };
renderQuests = function() { _rq(); renderBottomBar(); };
